function calculateDays() {
    ...
    if (isNaN(startDate.getTime())) {
        start.style.borderColor = '#ff5454';
        startError.style.fontSize = '10px'
        startError.style.display = 'block';
        startError.style.margin= 0;
        startError.style.marginTop= '10px'; // Mostra a mensagem de erro
        valid = false;
    } else {
        start.style.borderColor = '#131B3F'; // Resetar a borda para a cor padrão se válido
    }

    if (isNaN(endDate.getTime())) {
        end.style.borderColor = '#ff5454';
        endError.style.fontSize = '10px'
        endError.style.display = 'block';
        endError.style.margin= 0;
        endError.style.marginTop= '10px'; // Mostra a mensagem de erro
        valid = false;
    } else {
        end.style.borderColor = '#131B3F'; // Resetar a borda para a cor padrão se válido
    }
